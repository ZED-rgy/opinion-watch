...
- 已完成：规则判定、P0-P3 风险分级、人工复核队列和人工结论保护；每轮扫描
  成功后自动判定新增及更新内容。
- 已完成：搜索卡片先写入巡检候选台账，再由本地规则筛选；只有疑似或不确定内容才
  调用大模型复判，普通内容保留过滤原因并跳过大模型。模型失败时继续使用规则结果，
  当前只发送标题、正文和评论等文本证据。
- 已完成：品牌相关但未命中浅层风险的卡片采用跨轮轮换抽查，优先核查从未打开过
  详情的内容；默认抽查当前关键词结果的 25%，最多 10 条。
- 已完成：每日自动清理超过 90 天的已过滤候选，以及超过 30 天且未被审计记录引用的
  诊断文件；诊断目录默认限制为 1 GiB，正式内容和仍被引用的证据不受影响。
- 当前“速探长、优速卖、配达人”同时作为品牌名和主体公司名称，以精确关键词
  搜索；后续品牌仍通过数据库增删改，不写死在采集器中。
- 已完成：疑似/高风险内容的相似事件聚类、日报中的聚合事件统计，以及桌面系统托盘
  入口。托盘功能仅在正常桌面运行时启用，冒烟测试不会常驻托盘。
- 已完成：设置页可显式启用或关闭当前 Windows 用户的开机启动；默认关闭。
- 已提供 GitHub Actions 桌面打包流程：手动运行或推送 `v*` 标签后，生成 Windows
  和 macOS 压缩包；当前产物未签名、未公证，且运行时仍需要本机安装 Google Chrome。
- 当前采集依赖平台页面结构；选择器失效、验证码或访问频率限制均需要人工介入。
- 已完成桌面基础版：6个桌面页面、品牌多关键词管理、多平台账号记录、账号独立
  自动巡检登录档案、应用内舆情中心、人工复核、未读播报和应用内定时巡检。
- 已完成企微智能机器人日报基础能力：当天首次成功的定时巡检或 Agent 导入后生成一条日报，手动巡检每次成功后发送一条，
...
- 已完成 Agent 候选 JSONL 导入：外部执行器只负责发现线索，系统继续负责候选留痕、
  规范化去重、分类和人工复核；没有详情证据的候选不会直接生成 P1/P2 播报。
...
私人仓库的 Actions 页面中运行 `Desktop packages`，完成后在该次运行的 Artifacts
中下载 `OpinionWatch-windows` 或 `OpinionWatch-macos`。首次使用时先解压并启动，
...
搜索结果轻量候选先进行本地规则初筛，只对疑似或不确定内容调用大模型并打开详情页；普通内容会
...
但当前版本不会把这些媒体发送给大模型。默认使用有界面浏览器，POC阶段不建议使用
`--headless`。采集默认使用 1 个页面；可用 `--concurrency 2` 到 `--concurrency 4`
为同一登录档案开启多个独立采集页面。并发页面共享同一账号会话，过高可能触发平台验证，
因此桌面端默认保持 1，建议先从 2 开始小范围验证。
...
Codex Agent、浏览器 Agent 或其他合规数据源可以把发现结果输出为 UTF-8 JSONL，再由
系统通过正式入口导入。外部执行器不得直接修改 SQLite：

```powershell
uv run opinion-watch ingest .\agent-results.jsonl
```

每行至少包含 `platform`、`brand`、`url`，以及 `title` 或 `snippet`。`relevance` 可选
`relevant`、`uncertain`、`irrelevant`，默认按 `uncertain` 处理；只有执行器确实读取了
正文或其他详情证据时才应设置 `detail_collected: true`。完整示例见
[`docs/examples/agent-results.jsonl`](docs/examples/agent-results.jsonl)。

导入会建立来源为 `agent` 的巡检批次，保留全部候选并按平台内容 ID 或规范化 URL
幂等去重。相关性不确定或只有搜索卡片证据的内容最多进入 P3 待调查队列；只有具备
详情证据的 P1/P2 才会产生“舆情提醒”。登录、覆盖不足等异常属于“系统运行”通知，
桌面播报页可按通知类型筛选。
...
测试连接时只探测文本消息能力，并把结果缓存到本地配置中。当前版本固定使用文本复判，
图片、视频和关键帧只作为采集证据保存，不会发送给大模型。

大模型复判不是所有搜索结果的必经步骤：系统先用规则筛掉普通帖子，只把规则命中、
不确定或需要人工复核的候选送入模型；模型超时或返回格式异常时保留规则结果并生成告警，
不会阻塞整次巡检。单次请求最多等待 30 秒，候选请求最多并行 3 个。
...
uv run mypy src/opinion_watch/events.py src/opinion_watch/scheduling.py src/opinion_watch/services.py
uv run pytest
```
...
日报在 `watch` 定时巡检或 Agent 导入当天首次成功/部分成功后发送一次；手动“立即巡检”每次成功或部分成功都会发送一条；
同一天后续巡检不会重复发送。
